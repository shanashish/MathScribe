...
def analyze_image(img: Image, dict_of_vars: dict):
    ...
    print(raw_text)
    bracketed_text = re.search(r"\[.*\]", raw_text, re.DOTALL)
    answers = []
    if bracketed_text:
        try:
            answers = ast.literal_eval(bracketed_text.group(0))
        except Exception as e:
            print(f"Error in parsing response: {e}")
    else:
        print("No bracketed JSON-like content found in Gemini response.")
